        ...
        if (this.resultData) {
          if (index >= 0 ) {
            if (this.resultData[index][this.config['lableName']]) {
              this._valuetext = this.resultData[index][this.config['lableName']];
            }
            backValue['dataItem'] = this.resultData[index]
          } else {
            // 取值
            const componentvalue = {};
            componentvalue[this.config['valueName'] ? this.config['valueName'] : 'Id'] = name;
            if (this.config.ajaxConfig) {
              const backselectdata =  await this.table.loadByselect(this.config.ajaxConfig, componentvalue, this.bsnData, this.casadeData);
              if (backselectdata.hasOwnProperty(this.config['lableName'] ? this.config['lableName'] : 'name')) {
                this._valuetext = backselectdata[this.config['lableName'] ? this.config['lableName'] : 'name']; 
              } else {
